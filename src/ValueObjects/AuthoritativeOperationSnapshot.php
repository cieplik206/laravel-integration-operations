...
/** @api */
final readonly class AuthoritativeOperationSnapshot
{
    ...
    public function __construct(
        ...
        public ?TerminalProofKind $terminalProofKind = null,
        private ?EncodedResult $encodedResult = null,
    ) {
        if (! $operationType->belongsTo($scope->provider)) {
            throw new InvalidArgumentException('Operation type does not belong to the authoritative snapshot provider.');
        }
     ...
     *     disposition: string,
     *     effect_state: string,
     *     terminal_proof_kind: string|null,
    ...
    public function toArray(): array
    {
        return [
            ...
            'terminal_proof_kind' => $this->terminalProofKind?->value,
    ...
    public static function fromArray(
        ...
        if (($data['version'] ?? null) !== self::Version) {
            throw new InvalidArgumentException('Unsupported authoritative operation snapshot version.');
        }
        ...
        if (! is_array($scope)
            ...
            || $safeFailure !== null && ! is_array($safeFailure)) {
            throw new InvalidArgumentException('Authoritative operation snapshot envelope is invalid.');
        }
        ...
        $effectState = self::effectStateFromEnvelope($data);
        $terminalProofKind = self::terminalProofKindFromEnvelope($data);
        ...
        $snapshot = new self(
            ...
            terminalProofKind: $terminalProofKind,
            encodedResult: $encodedResult,
        );

        if (($data['disposition'] ?? null) !== $snapshot->disposition->value) {
            throw new InvalidArgumentException('Authoritative snapshot disposition does not match its status.');
        }
    ...
    public function equals(self $other): bool
    {
        return $this->operationId->equals($other->operationId)
            ...
            && $this->terminalProofKind === $other->terminalProofKind
    ...
    private function assertResultInvariant(): void
    {
        ...
        if ($this->resultAvailability === ResultAvailability::Available
            && ($this->result === null || $this->encodedResult === null)) {
            throw new InvalidArgumentException('Available authoritative operation result is missing.');
        }

        if (in_array($this->resultAvailability, [ResultAvailability::CodecUnavailable, ResultAvailability::DecodeFailed], true)
            && ($this->result !== null || $this->encodedResult === null)) {
            throw new InvalidArgumentException('Undecodable authoritative result must retain only its encoded envelope.');
        }

        if (in_array($this->resultAvailability, [ResultAvailability::NotReady, ResultAvailability::NotApplicable], true)
            && ($this->result !== null || $this->encodedResult !== null)) {
            throw new InvalidArgumentException('Authoritative snapshot without a result must not expose an envelope.');
        }

        if ($this->result !== null
            && $this->encodedResult !== null
            && $this->result->resultType() !== $this->encodedResult->resultType) {
            throw new InvalidArgumentException('Decoded authoritative result type does not match its envelope.');
        }

        if (! $this->isTerminal() && $this->resultAvailability !== ResultAvailability::NotReady) {
            throw new InvalidArgumentException('A non-terminal authoritative snapshot cannot expose a terminal result.');
        }

        if ($this->isTerminal() && $this->resultAvailability === ResultAvailability::NotReady) {
            throw new InvalidArgumentException('A terminal authoritative snapshot cannot have a not-ready result.');
        }

        if ($this->status === OperationStatus::Succeeded
            && ! in_array($this->resultAvailability, $encodedAvailability, true)) {
            throw new InvalidArgumentException('A succeeded authoritative operation must retain a result envelope.');
        }

        if (! in_array($this->status, [OperationStatus::Succeeded, OperationStatus::Failed], true)
            && in_array($this->resultAvailability, $encodedAvailability, true)) {
            throw new InvalidArgumentException('Only a succeeded or failed authoritative operation may expose a result.');
        }

        if ($this->status === OperationStatus::Cancelled
            && $this->resultAvailability !== ResultAvailability::NotApplicable) {
            throw new InvalidArgumentException('A cancelled authoritative operation cannot expose a result.');
        }
    }
    ...
    private function assertFailureInvariant(): void
    {
        if ($this->status === OperationStatus::Failed && $this->safeFailure === null) {
            throw new InvalidArgumentException('A failed authoritative operation must expose a safe failure.');
        }

        if (in_array($this->status, [OperationStatus::Succeeded, OperationStatus::Cancelled], true)
            && $this->safeFailure !== null) {
            throw new InvalidArgumentException('A succeeded or cancelled authoritative operation cannot expose a failure.');
        }
    }

    private function assertProofInvariant(): void
    {
        if ($this->isTerminal() !== ($this->terminalProofKind !== null)) {
            throw new InvalidArgumentException('Authoritative terminal proof presence does not match operation status.');
        }
    }
    ...
    private static function requiredString(array $data, string $key): string
    {
        ...
        if (! is_string($value)) {
            throw new InvalidArgumentException("Authoritative snapshot field '{$key}' is invalid.");
        }
    ...
    private static function assertExactKeys(array $data, array $expectedKeys, string $field): void
    {
        if (array_diff(array_keys($data), $expectedKeys) !== []
            ...
            throw new InvalidArgumentException("Authoritative snapshot {$field} fields are invalid.");
        }
    }
    ...
    {
        return OperationStatus::tryFrom(self::requiredString($data, 'status'))
            ?? throw new InvalidArgumentException('Authoritative snapshot status is invalid.');
    }

    /** @param array<string, mixed> $data */
    private static function effectStateFromEnvelope(array $data): EffectState
    {
        return EffectState::tryFrom(self::requiredString($data, 'effect_state'))
            ?? throw new InvalidArgumentException('Authoritative snapshot effect state is invalid.');
    }

    /** @param array<string, mixed> $data */
    private static function terminalProofKindFromEnvelope(array $data): ?TerminalProofKind
    {
        $value = $data['terminal_proof_kind'] ?? null;
        ...
        if (! is_string($value)) {
            throw new InvalidArgumentException('Authoritative snapshot proof kind is invalid.');
        }

        return TerminalProofKind::tryFrom($value)
            ?? throw new InvalidArgumentException('Authoritative snapshot proof kind is invalid.');
    }
    ...
    {
        return ResultAvailability::tryFrom(self::requiredString($data, 'result_availability'))
            ?? throw new InvalidArgumentException('Authoritative snapshot result availability is invalid.');
    }
    ...
    private static function assertScopeEnvelope(array $scope): void
    {
        if (($scope['version'] ?? null) !== IntegrationScope::Version
            ...
            throw new InvalidArgumentException('Authoritative snapshot scope values are invalid.');
        }
    }
    ...
    private static function assertContextEnvelope(array $context): void
    {
        ...
        if (($context['version'] ?? null) !== IntegrationContext::Version
            ...
            || ! is_array($attributes)) {
            throw new InvalidArgumentException('Authoritative snapshot context values are invalid.');
        }

        foreach ($attributes as $key => $value) {
            if (! is_string($key)
                || ! is_string($value) && ! is_int($value) && ! is_bool($value) && $value !== null) {
                throw new InvalidArgumentException('Authoritative snapshot context attributes are invalid.');
            }
        }
    }
    ...
    private static function assertResultEnvelope(array $result): void
    {
        ...
        if (! is_string($result['result_type'] ?? null)
            ...
            throw new InvalidArgumentException('Authoritative snapshot result values are invalid.');
        }
    }
    ...
    private static function assertSafeFailureEnvelope(array $safeFailure): void
    {
        if (($safeFailure['version'] ?? null) !== SafeOperationFailure::Version
            ...
            throw new InvalidArgumentException('Authoritative snapshot safe failure values are invalid.');
        }
    }
